  ...
  return (
    <div>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger
          className="w-full p-2 pl-5 flex items-center rounded-md cursor-pointer bg-black
         text-white hover:bg-black transition-all duration-200 disabled:text-sm disabled:opacity-50 
         disabled:cursor-not-allowed "
          disabled={!user}
        >
          <Plus className="w-4 h-4 mr-2" />
          {user ? "create cult" : "login to create a cult"}
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>create a cult</DialogTitle>
            <DialogDescription>
              create a cult for ur fellow fetish schizos.
            </DialogDescription>

            <form onSubmit={handleCreateCommunity} className="space-y-4 mt-2">
              {errorMessage && (
                <div className="mb-5 bg-red-500 text-white p-2 rounded-md">
                  {errorMessage}
                </div>
              )}
